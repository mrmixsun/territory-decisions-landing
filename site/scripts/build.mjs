...
function landingPageS11() {
  ...
  const body = `<section class="s11-hero" id="hero"><div class="container s11-hero__grid"><div class="s11-hero__copy"><span class="s11-kicker">${e(hero.eyebrow)}</span><h1>${heroTitle}</h1><p>${heroLead}</p></div><figure class="s11-hero__visual"><img src="${asset('hero-imt-ecosystem-blue.png')}" alt="${e(hero.visualCaption)}"><figcaption>Территория · данные · решение</figcaption></figure></div></section>
  <section class="s11-section s11-section--mist s11-story" id="story"><div class="container">${sectionTitle(story.eyebrow, story.title, story.intro)}<div class="s11-story-shell"><div class="s11-story-stage">${slides}</div><div class="s11-story-controls"><div class="s11-story-tabs" role="tablist" aria-label="Четыре шага объяснения Концепции">${tabs}</div><div class="s11-story-arrows"><button type="button" data-story-prev aria-label="Предыдущий слайд">←</button><button type="button" data-story-next aria-label="Следующий слайд">→</button></div></div></div><p class="s11-footnote">${e(story.videoNote)}</p></div></section>
